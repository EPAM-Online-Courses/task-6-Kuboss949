...
class PeopleProcessor {
    ...
    List<String> namesOfPeopleWhoseAgeIsGreaterThan(int age, Collection<Person> people) {
        return people.stream().filter(p -> p.getAge()>age).map(p-> p.getName()).toList();
    }
    ...
    List<String> namesOfPeopleSortedByAgeLivingIn(String cityOfLiving, Collection<Person> people) {
        return people.stream().filter(p -> p.getCityOfLiving()==cityOfLiving).sorted(Comparator.comparing(Person::getAge)).map(p -> p.getName()).toList();
    }
    ...
    Map<String, Double> averageAgeByCityOfLiving(Collection<Person> people) {
        return people.stream().collect(Collectors.groupingBy(p -> p.getCityOfLiving(), Collectors.averagingDouble(Person::getAge)));
    }
}
